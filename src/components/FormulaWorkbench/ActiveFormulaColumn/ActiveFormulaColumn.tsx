...
import { Dropdown, DropdownMenuOption, useToast } from "../../common";
...
import YieldingModal, { YieldingOptions } from "../modals/YieldingModal";
import NormalizeFormulaModal from "../modals/NormalizeFormulaModal";
import {
  ActiveGroupedColumn,
  ColumnHeader,
...
const ActiveFormulaColumn: React.FC<ActiveFormulaColumnProps> = ({
  ...
  const [showYieldingModal, setShowYieldingModal] = useState(false);
  ...
  const dropdownMenuItems: DropdownMenuOption[] = [
    ...
    },
    {
      ...
      icon: "calculator",
      iconColor: "secondary",
      action: () => setShowYieldingModal(true),
    },
    {
      id: "round-off",
      label: "Round Weights",
      ...
      action: () => {
        roundOffIngredients();
        showSuccess("Round Off complete", 2000);
      },
    },
    {
      id: "normalization",
      label: "Normalize Totals",
      icon: "balanceScale",
      iconColor: "secondary",
      action: () => setShowNormalizeModal(true),
    },
    {
      id: "create-version",
  ...
  const handleAmountChange = (id: string, amount: number) => {
    // For now, we'll just update the quantity without concentration calculation
    // since batch size is moving to the summary section
    onUpdateIngredient(id, { quantity: amount });
  };

  const handleYieldingApply = (options: YieldingOptions) => {
    applyYielding(options);
    showSuccess(
      `Yielded to ${options.targetYield}${options.targetUnit.toLowerCase()}`,
      2000
    );
  };
    ...
    <ActiveGroupedColumn>
      ...
      <ActiveTableContainer>
        ...
        >
          ...
          <TableHeaderCell title="Cost per kilogram in euros">
            Cost (€/kg)
          </TableHeaderCell>
                  ...
                  if (item.isMissing) {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell
                          title={`Amount: ${formulaIngredient.quantity || 0}g`}
                        >
                          <EditableInput
                            type="number"
                            value={formulaIngredient.quantity || 0}
                            onChange={(
                        ...
                        <TableCell
                          title={`Total contribution: €${calculateContributionCost(formulaIngredient, formulaIngredient.quantity || 0).toFixed(3)}`}
                        >
                          €
                          {calculateContributionCost(
                            formulaIngredient,
                            formulaIngredient.quantity || 0
                          ).toFixed(3)}
                        </TableCell>
                        <TableCell
                          style={{ textAlign: "center" }}
      ...
      {/* Yielding Modal */}
      <YieldingModal
        isOpen={showYieldingModal}
        onClose={() => setShowYieldingModal(false)}
        onApply={handleYieldingApply}
        activeIngredients={getAllIngredients()}
        currentBatchSize={batchSize}
        currentUnit={batchUnit}
          ...
          {searchResults.map((ingredient) => (
            ...
            >
              ...
              <div style={{ fontSize: "12px", color: "#6b7280" }}>
                {ingredient.category} • €
                {(ingredient.costPerKg || 0).toFixed(3)}/kg
              </div>
            </SearchDropdownItem>
          ))}
        </SearchDropdown>
